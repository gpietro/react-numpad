...
test('decimal with negative', () => {
  const { getByText, getByRole, container } = render(<NumPad.Number inline decimal={2} />);
...
test('minus sign', () => {
  const { getByRole, container } = render(<NumPad.Number inline />);
