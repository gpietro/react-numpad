...
function NumPad({
  ...
  const [value, setValue] = useState((receivedValue || '').toString());
  const [preValue, setPreValue] = useState();
  const [inputCoords, setInputCoords] = useState();

  if (receivedValue !== preValue) {
    setPreValue(receivedValue);
    setValue((receivedValue || '').toString());
  }
...
NumPad.propTypes = {
  ...
  receivedValue: PropTypes.oneOfType([PropTypes.number, PropTypes.string]),
  sync: PropTypes.bool,
