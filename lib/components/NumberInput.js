...
import { KeyPad, Wrapper } from '../elements';
...
const KeyPadEditor = props => {
  const { theme, position, onChange } = props;
  const customTheme = typeof theme === 'object' ? theme : styles(theme);
  customTheme.position = position;
  return (<ThemeProvider theme={customTheme}>
  <KeyPad {...defaultProps(props)} confirm={onChange} />
  </ThemeProvider>)
}
export {NumberInput, KeyPadEditor}
