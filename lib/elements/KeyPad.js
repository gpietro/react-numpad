...
const KeyPad = ({
  ...
  if (sync && prevProps !== value && validation(value)) {
    setPrevProps(value);
  ...
  return (
    <Content>
      <Header>
        ...
        <Label>{label}</Label>
        <NButton onClick={() => confirm(displayRule(value))} disabled={!validation(value)}>
          {validation(value) ? <IconCheckCircle /> : <IconCheck />}
        </NButton>
      </Header>
      <Display value={value} displayRule={displayRule} cancel={() => update(value.slice(0, -1))} />
      <Keys>
