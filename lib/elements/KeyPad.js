...
class KeyPad extends Component {
    ...
    handleClick(key) {
        let {float, negative, keyValid} = this.props
    ...
    numericKeyClick(keyPressed) {
        if( this.props.keyValid(this.state.input, keyPressed ) ) {
    ...
    render() {
        let { float, negative, displayRule, validation, label, confirm, cancel, theme, keyValid } = this.props
                    ...
                    <Keys>
                        {[7, 8, 9, 4, 5, 6, 1, 2, 3, '-', 0, '.'].map( key => (
                            <Button
                                ...
                                value={key}
                                disabled={!activeKeys.includes(key) || !keyValid(this.state.input, key)} />
                        ))}
                    </Keys>
                </Content>
            </Container>
