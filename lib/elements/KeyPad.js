...
class KeyPad extends Component {
  ...
  handleClickOutside(evt) {
    evt.preventDefault();
    evt.stopPropagation();
    const { validation, confirm, displayRule, dateFormat, cancel } = this.props;
    const { input } = this.state;
    if (validation(input)) {
  ...
  handleClick(key) {
    ...
    if (keyValid(this.state.input, key, keyValidator || dateFormat)) {
      if (key === '-') {
    ...
    return (
      <Content>
        <Header>
          ...
          <Label>{label}</Label>
          <NButton
            onClick={() => confirm(displayRule(this.state.input, dateFormat))}
        ...
        <Keys>
          {[7, 8, 9, 4, 5, 6, 1, 2, 3, '-', 0, '.'].map(key => (
            <Button
              ...
              value={key}
              disabled={!keyValid(this.state.input, key, keyValidator || dateFormat)}
            />
          ))}
        </Keys>
      </Content>
