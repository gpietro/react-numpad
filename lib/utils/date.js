...
const display = (value, dateFormat) => {
  value += '_'.repeat(
    Math.max(0, dateFormat.replace(/[^a-z]/gi, '').length - value.length)
  );
  ...
  return dateFormat
    ...
    .reduce(
      (p, c, i) =>
        p.length > 0 ? [...p, [p[i - 1][1], c + p[i - 1][1]]] : [[0, c]],
      []
    )
    .map(s => value.substring(s[0], s[1]))
    .join(symbol);
};

const validate = (value, key, dateFormat) => {
  let checkDate = moment(VALIDATION_DATE, VALIDATION_FORMAT).format(
